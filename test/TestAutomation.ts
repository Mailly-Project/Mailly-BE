...
import { GlobalConfig } from "../src/config/app.config";
...
export namespace TestAutomation {
  ...
  export const execute = async <T>(props: IProps<T>): Promise<void> => {
    ...
    if (options.reset) {
      await StopWatch.trace("Reset DB")(Setup.schema);
      await StopWatch.trace("Seed Data")(Setup.seed);
    }

    // DO TEST
    const backend: T = await props.open(options);
    const connection: api.IConnection = {
      host: `http://127.0.0.1:${GlobalConfig.API_PORT()}`,
    };
